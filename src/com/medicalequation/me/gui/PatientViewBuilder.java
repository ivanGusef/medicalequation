...
public class PatientViewBuilder {
    ...
    private final Map<String, View> mutableHolder = new HashMap<String, View>();
    private final Map<String, View> immutableHolder = new HashMap<String, View>();
    private final Map<String, View> resultHolder = new HashMap<String, View>();
    ...
    public void generate() {
        ...
        for (Line mutableLine : currentTherapy.mutableLines) {
            View mutableLineView = activity.getLayoutInflater().inflate(editMode ? mutableLine.type.editRes : mutableLine.type.viewRes, null);
        ...
        for (Line immutableLine : currentTherapy.immutableLines) {
            View immutableLineView = activity.getLayoutInflater().inflate(editMode ? immutableLine.type.editRes : immutableLine.type.viewRes, null);
        ...
        for (Line resultLine : currentTherapy.resultLines) {
            View resultLineView = activity.getLayoutInflater().inflate(editMode ? resultLine.type.editRes : resultLine.type.viewRes, null);
    ...
    public Map<String, Number> getMutableValues() {
        ...
        for (Line mutableLine : currentTherapy.mutableLines) {
            ...
            value = getByType(editor, mutableLine);
    ...
    public Map<String, Number> getImmutableValues() {
        ...
        for (Line immutableLine : currentTherapy.immutableLines) {
            ...
            value = getByType(editor, immutableLine);
    ...
    public Map<String, Number> getResultValues() {
        ...
        for (Line resultLine : currentTherapy.resultLines) {
            ...
            value = getByType(editor, resultLine);
    ...
    public boolean validateAll(Map<String, Number> mutableValues, Map<String, Number> immutableValues, Map<String, Number> resultValues) {
        ...
        for (Line mutableLine : currentTherapy.mutableLines) {
            ...
            if ((validateErrorMessage = mutableLine.validate(activity, value)) != null) {
                ((TextView) editor).setError(validateErrorMessage);
                valid = false;
            }
        }
    ...
    private void setByType(Number value, Map<String, View> holder, Line line) {
        switch (line.type) {
            case INT:
            case REAL:
                ((TextView) holder.get(line.name)).setText(value != null ? String.valueOf(value) : null);
                break;
            case SELECT:
                if (editMode) {
                    ((Spinner) holder.get(line.name)).setSelection(value != null ? value.intValue() - 1 : 0);
                } else {
                    ((TextView) holder.get(line.name)).setText(value != null ? line.choiceItems[value.intValue() - 1] : line.choiceItems[0]);
                }
                break;
            default:
                throw new IllegalArgumentException("Unknown line type " + line.type);
        ...
        if (editMode) {
            switch (line.type) {
                ...
                case REAL:
                    ...
                    break;
                case SELECT:
                    ((Spinner) editor).setAdapter(new SelectAdapter(activity, line.choiceItems));
                    break;
                default:
                    throw new IllegalArgumentException("Unknown line type " + line.type);
            }
    ...
    public static class SelectAdapter extends ArrayAdapter<String> {
